...
def evaluate_function(ourSpareHero, ourChosenHero, enemyChosenHero,wholeChart):
    ...
    antiKey=dict(zip(ourSpareHero, antiEnemyHeroScore))
   ...
   # print('antiEnemyHeroList',antiEnemyHeroList)
    ...
    EnemyHeroRestrainKey=dict(zip(ourSpareHero, antiEnemyHeroScore))
    for i in range(len(enemyChosenHero)):
        name = enemyChosenHero[i]
        heroData = data_process.get_data_from_name(wholeChart,name)
        if heroData['克制英雄1'] in EnemyHeroRestrainKey:
        ...
        if heroData['克制英雄2'] in EnemyHeroRestrainKey:
        ...
        if heroData['克制英雄3'] in EnemyHeroRestrainKey:
   ...
   # print('EnemyHeroRestrainkey',EnemyHeroRestrainKey)
    ...
    return finalRecommandHero, ourSpareHero      #在核心函数中不直接将推荐的英雄更新的新选英雄中，将这一权力留给玩家

def update(finalChoseHero,ourSpareHero,ourChosenHero,wholeChart):
    heroData=data_process.get_data_from_name(wholeChart,finalChoseHero)
    branch = heroData['分路']
    #print('branch',branch)
    ...
    ourChosenHero.append(finalChoseHero)
    #print('newourChosenHero',ourChosenHero)

    return newOurSpareHero,ourChosenHero
...
if __name__=='__main__':
    ...
    ourSpareHero =['狂铁', '梦奇', '铠', '杨玉环', '女娲', '干将莫邪', '李元芳', '后羿', '狄仁杰']
    enemyChosenHero=['鲁班七号','孙悟空']
    ourChosenHero=['盾山','李白']
    finalRecommandHero, newOurSpareHero, ourChosenHero=evaluate_function(ourSpareHero, ourChosenHero, enemyChosenHero,wholeChart)
